<template>
	<view>
		<u-navbar :is-back="true" title="单位公告" :border-bottom="false" back-icon-color="#000" :background="background"
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(data) {
				uni.navigateTo({
					url: "/pages/my/gonggao/detail?data="+encodeURIComponent(JSON.stringify(data))
				})
			},
			addBtn() {
				this.$navigateTo({
					url: "/pages/my/gonggao/addPage"
				}).then(res=>{
					this.page=1;
					this.getListData()
				})
