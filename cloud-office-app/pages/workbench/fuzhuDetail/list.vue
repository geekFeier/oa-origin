<template>
	<view>
		<u-navbar :is-back="true" :border-bottom="false" back-icon-color="#000" :background="background"
			title-color="#000" title="辅助核算明细账" :height="55">
		</u-navbar>
		<view class="header-div" @click="openPopup">
			<text
				class="date-text">{{formData.year}}年{{formData.month}}期-{{formData.to_year}}年{{formData.to_month}}期</text>
		...
		<view class="mainBox">
			<view class="main-item" v-for="(item,index) in listData" :key="index">
				...
				<view class="main-item-r">
					...
					<view class="main-item-r-line">
						<view class="main-item-r-line-l">
							余额 <view class="main-item-one-m"
								:class="{blue:item.lender===item.borrower,red:item.lender!==item.borrower}">
...
<script>
	...
	export default {
		...
		onLoad(params) {
			...
			this.formData.hs_cate_id = data.pid;
			this.formData.member = data.id;
			this.formData.year = dayjs().year();
		...
		methods: {
				...
				this.$http("enterprise.date_query/getQueryTime", "get").then(res => {
					if (res.data.code == 1) {
						...
						let currentData = list.find(item => {
							return item.years === this.formData.year
						})
			...
			getListData() {
				this.$http("enterprise.Hesuan_balance/hs_dateils_list", this.formData, "get").then(res => {
					if (res.data.code == 1) {
