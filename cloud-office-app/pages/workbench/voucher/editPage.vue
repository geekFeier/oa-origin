<template>
	<view>
		<u-navbar :is-back="true" title="编辑凭证" :border-bottom="false" back-icon-color="#000" :background="background"
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			console.log(detail, "-----------------------------------");
			this.formData.recording_time = `${detail.years}-${Math.abs(detail.month)}-${detail.day}`;
			this.formData.years = detail.years;
			this.formData.month = Math.abs(detail.month);
			this.formData.day = detail.day;
				...
				return {
					...
					hesuan_children: item.childs.map(_item => {
						return {
							...
							hs_id: _item.hs_id,
							members: _item.hs_members_id,
							memberName: _item.members.name
						}
					})
				}
			})
		...
		methods: {
			...
			sureBtn() {
				...
				this.formData.credentials = JSON.stringify(this.credentials);
