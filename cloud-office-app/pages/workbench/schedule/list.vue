<template>
	<view class="generationOfficeBox">
		<u-navbar :is-back="true" :border-bottom="false" back-icon-color="#000" :background="background"
			title-color="#000" title="日程安排" :height="55">
		</u-navbar>

		<view class="mainBox">
			<view class="main-item" v-for="(item,index) in listData" :key="index" @click="goDetail(item.id,item)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(id,item) {
				uni.navigateTo({
					url: "/pages/workbench/schedule/detail?id=" + id+"&data="+JSON.stringify(item)
				})
			},
			goAddPage() {
				this.$navigateTo({
					url: "/pages/workbench/schedule/index"
				}).then(res => {
					this.page=1;
					this.listData = [];
					this.getList();
