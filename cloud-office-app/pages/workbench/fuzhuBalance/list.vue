<template>
	<view>
		<u-navbar :is-back="true" :border-bottom="false" back-icon-color="#000" :background="background"
			title-color="#000" title="辅助核算余额表" :height="55">
		</u-navbar>
		...
		<view class="mainBox">
			<view v-if="listData.length>0">
				<view class="main-item" v-for="(item,index) in listData" :key="index">
					<view class="main-item-header">
						<text>{{item.years}} {{item.kemu.name}}</text>
					...
					<view class="main-item-m">
						...
						<view class="main-item-m-n" v-if="item.borrow_early_period_balance">
							{{item.borrow_early_period_balance ? item.borrow_early_period_balance :0}}
						</view>
						<view class="main-item-m-n " v-if="item.loan_early_period_balance">
							{{item.loan_early_period_balance ? item.loan_early_period_balance : 0}}
						</view>
					</view>
					<view class="main-item-m">
						<view class="main-item-m-n">
						...
						<view class="main-item-m-n " v-if="item.borrow_end_period_balance">
							{{item.borrow_end_period_balance ? item.borrow_end_period_balance : 0}}
						</view>
						<view class="main-item-m-n " v-if="item.loan_self_period_balancece">
							{{item.loan_self_period_balancece ? item.loan_self_period_balancece:0}}
						</view>
					</view>
					<view class="main-item-m">
						<view class="main-item-m-n">
						...
						<view class="main-item-m-n " v-if="item.loan_end_period_balance">
							{{item.loan_end_period_balance ? item.loan_end_period_balance : 0}}
						</view>
					</view>
				</view>
			</view>
			...
			<!-- 		<view class="main-item">
				...
				<view class="main-item-m">
					...
					<view class="main-item-m-n " v-if="totalData.borrow_early_period_balance">
						{{totalData.borrow_early_period_balance ?totalData.borrow_early_period_balance:0 }}
					</view>
					<view class="main-item-m-n " v-if="totalData.loan_early_period_balance">
						{{totalData.loan_early_period_balance ? totalData.loan_early_period_balance:0}}
					</view>
				</view>
				<view class="main-item-m">
					<view class="main-item-m-n">
					...
					<view class="main-item-m-n ">
						{{totalData.borrow_self_period_balance?totalData.borrow_self_period_balance:0}}
					</view>
					<view class="main-item-m-n ">
						{{totalData.loan_self_period_balancece ? totalData.loan_self_period_balancece: 0}}
					</view>
				</view>
				<view class="main-item-m">
					<view class="main-item-m-n">
					...
					<view class="main-item-m-n ">
						{{totalData.borrow_end_period_balance ? totalData.borrow_end_period_balance:0}}
					</view>
					<view class="main-item-m-n ">
						{{totalData.loan_end_period_balance ? totalData.loan_end_period_balance: 0}}
					</view>
				</view>
			</view> -->
		</view>
	...
	export default {
		...
		methods: {
			...
			getListData() {
				...
				this.$http("enterprise.Hesuan_balance/index", this.formData, "get").then(res => {
					if (res.data.code == 1) {
