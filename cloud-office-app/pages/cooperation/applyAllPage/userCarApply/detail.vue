<template>
	<view>
		...
		<view class="header-div">
			<view class="header-t">{{detailData.user.username}}提交的用车申请</view>
		...
		<view class="main-box">
			<view class="mainBox">
				...
				<view class="form-item">
					...
					<view class="form-item-value">{{detailData.origin_locaiton}}</view>
				</view>
				<view class="form-item">
					<view class="form-item-title">
						返回地点
					</view>
					<view class="form-item-value">{{detailData.rollback_location}}</view>
				</view>
				<view class="form-item">
					<view class="form-item-title">
						用车日期
					</view>
					<view class="form-item-value">{{detailData.begin_time}}</view>
				</view>
				<view class="form-item">
					<view class="form-item-title">
						返回日期
					</view>
					<view class="form-item-value">{{detailData.end_time}}</view>
				</view>

				<view class="car-div-title">
					车辆明细({{detailData.vihicel.length}})
				</view>
				<view class="form-item4">
					<view class="form-item4-div" v-for="(_item,index) in detailData.vihicel" :key="index">
						<view class="form-item">
							<view class="form-item-title">
								车辆类型
							</view>
							<view class="form-item-value">{{_item.vehicle_type}}</view>
						</view>
						<view class="form-item">
							<view class="form-item-title">
								数量
							</view>
							<view class="form-item-value">{{_item.num}}</view>
						</view>
						<view class="form-item">
							<view class="form-item-title">
								其他要求
							</view>
							<view class="form-item-value">{{_item.other}}</view>
						</view>
	...
	export default {
		...
		methods: {
			refuseBtn() {
				...
				this.$http("enterprise.applyfor.Yongche/operaction", params, "post").then(res => {
					if (res.data.code == 1) {
						uni.showToast({
							title: "成功",
							icon: "none"
