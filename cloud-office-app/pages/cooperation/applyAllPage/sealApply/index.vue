<template>
	<view class="">
		<u-navbar :is-back="true" title="用印申请" :border-bottom="false" back-icon-color="#000" :background="background"
		...
		<view class="mainBox" v-if="swiperCurrent==0">
			...
			<view class="form-item">
				...
				<u-input v-model="formData.stamp" @click="voucherPopupBtn" type="select" :clearable="false"
					placeholder="请选择" />
			...
			<view class="main-item" @click="goDetail(item)" v-for="(item,index) in listData" :key="index">
				...
				<view class="main-item-m">
					<view class="main-item-m-text">
						经办人：{{item.receivi_person}}
					</view>
					<view class="main-item-m-text">
						日期：{{item.use_time}}
					</view>
					<view class="main-item-m-text">
						用印文件名称：{{item.use_file_name}}
					</view>
					<!-- 	<view class="main-item-m-text">
						文件类别：
					</view> -->
		...
		<u-popup v-model="showVoucherPopup" mode="bottom" border-radius="56">
			<view class="popup-main">
				<view class="popup-common-title">
					— 加盖何种印章 —
				</view>

				<view class="popup-common-m">
				...
				<view class="popup-common-btnGroup">
					...
					<view class="popup-common-btn active" @click="sureSelectType">
						确定
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			getListData() {
				...
				this.$http("enterprise.applyfor.Yongyin/index", params, "get").then(res => {
