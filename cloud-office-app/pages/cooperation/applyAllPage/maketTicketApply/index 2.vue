<template>
	<view class="">
		<u-navbar :is-back="true" title="开票申请" :border-bottom="false" back-icon-color="#000" :background="background"
		...
		<view class="mainBox" v-if="swiperCurrent==0">
			<view class="form-item">
				...
				<u-input v-model="formData.content" type="textarea" :auto-height="true" :clearable="false" placeholder="请输入开票事由" />
			</view>
			...
			<view class="form-item">
				...
				<u-input v-model="formData.remarks" type="text" :auto-height="true" :clearable="false" placeholder="请输入" />
			</view>
		...
		<view class="mainBox2" v-if="swiperCurrent==1">
			<view class="main-item" v-for="(item,index) in listData" :key="index" @click="goDetail(item)">
				<view class="main-item-t">
					<view class="main-item-t-l">
						{{item.user.username}}提交的开票申请
					</view>
	...
	export default {
		...
		methods: {
			getReceiviPersion(){
				...
				this.$http("enterprise.applyfor.base/getReceiviPersion?pageId=3",params,"post").then(res=>{
			...
			getListData() {
				...
				this.$http("enterprise.applyfor.Kaipiao/index", params, "get").then(res => {
			...
			goDetail(item) {
				this.$navigateTo({
					url: "/pages/cooperation/applyAllPage/maketTicketApply/detail?data=" + encodeURIComponent(JSON
						.stringify(item))
				}).then(res => {
					this.page = 1;
					this.listData = [];
