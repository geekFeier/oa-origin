<template>
	<view>
		<u-navbar :is-back="true" title="申请开票" :border-bottom="false" back-icon-color="#000" :background="background"
			title-color="#000" :height="55">
		</u-navbar>
		<view class="header-div">
			<view class="header-t">{{detailData.user.username}}提交的开票申请</view>
	...
	export default {
		...
		methods: {
			refuseBtn() {
				...
				this.$http("enterprise.applyfor.Kaipiao/operaction", params, "post").then(res => {
					if (res.data.code == 1) {
						uni.showToast({
							title: "成功",
							icon: "none"
