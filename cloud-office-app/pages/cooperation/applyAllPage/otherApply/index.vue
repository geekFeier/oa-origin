	...
	export default {
		...
		methods: {
			getReceiviPersion(){
				...
				this.$http("enterprise.applyfor.base/getReceiviPersion?pageId=7",params,"post").then(res=>{
			...
			submitBtn() {
				...
				if (verify) {
					this.$http("enterprise.applyfor.Other/CreateForm", this.formData, "post").then(res => {
						if (res.data.code == 1) {
							this.formData = {};
							this.recipientList = [];
							this.copiedPeople = [];
			...
			getListData() {
				...
				this.$http("enterprise.applyfor.Other/index", params, "get").then(res => {
